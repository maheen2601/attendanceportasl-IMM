...
export default function AdminDashboard() {
  ...
  // 🔹 NEW: Shift-wise derived counts
  const shift8to4 = Number(stats.shift_8_4 ?? 0);
  const shift10to7 = Number(stats.shift_10_7 ?? 0);
  const shift12to8 = Number(stats.shift_12_8 ?? 0);
  const shift4to12 = Number(stats.shift_4_12 ?? 0);
  ...
  return (
    <div className="w-full min-h-screen bg-gradient-to-b from-gray-50 to-white px-6 py-10">
      ...
      {/* 🔹 NEW: Shift-wise cards (today) */}
<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
  <StatCard
    title="Shift 8am – 4pm (Today)"
    value={shift8to4}
    accent="from-blue-500/20 to-indigo-500/20"
    icon="shift"
    extra={
      <div className="mt-1 text-xs text-gray-500">
        Employees assigned & checked-in for this shift.
      </div>
    }
  />

  <StatCard
    title="Shift 10am – 7pm (Today)"
    value={shift10to7}
    accent="from-emerald-500/20 to-teal-500/20"
    icon="shift"
    extra={
...
function Icon({ kind, className }) {
  switch (kind) {
    case "users":
      return (
        <svg viewBox="0 0 24 24" className={className} fill="currentColor">
          <path d="M16 11c1.66 0 3-1.34 3-3S17.66 5 16 5s-3 1.34-3 3m-8 0c1.66 0 3-1.34 3-3S9.66 5 8 5S5 6.34 5 8s1.34 3 3 3m0 2c-2.33 0-7 1.17-7 3.5V20h14v-3.5C15 14.17 10.33 13 8 13m8 0c-.29 0-.62.02-.97.05c1.16.84 1.97 1.94 1.97 3.45V20h6v-3.5c0-2.33-4.67-3.5-7-3.5Z" />
        </svg>
      );
    case "check":
      return (
    ...
    case "shift":
      return (
        <svg viewBox="0 0 24 24" className={className} fill="currentColor">
          <path d="M12 2a9 9 0 1 0 9 9h-2a7 7 0 1 1-7-7V2m1 4h-2v5.41l3.3 3.3l1.4-1.42L13 10.59V6Z" />
        </svg>
      );
    default:
      return <span className={className} />;
