...
//                       openCorrectionFor(r)} className="text-indigo-600 hover:underline">
//                       Request time change
//                     </button>
//                   </td>
//                 </tr>
...
/* ---- mode helpers ---- */
const MODE_TAG_RE = /\[MODE:(WFH|Onsite)\]/i;
function decodeModeFromReason(reason) {
  if (!reason) return null;
  const m = String(reason).match(MODE_TAG_RE);
  return m ? (m[1].toUpperCase() === "WFH" ? "WFH" : "Onsite") : null;
}
// Also accept "mode: wfh" / "mode=onsite" styles
function decodeModeFromText(txt) {
  if (!txt) return null;
  const s = String(txt);
  let m = s.match(/\[MODE\s*:\s*(WFH|Onsite)\]/i);
  if (m) return m[1].toUpperCase() === "WFH" ? "WFH" : "Onsite";
  m = s.match(/\bmode\s*[:=]\s*(WFH|Onsite)\b/i);
  if (m) return m[1].toUpperCase() === "WFH" ? "WFH" : "Onsite";
  return null;
}
...
export default function MyAttendance() {
  ...
  // initial: this month, clamp "to" to today if current month
  useEffect(() => {
    const f = startOfMonth();
    const eom = endOfMonth();
    const t = new Date(eom) > new Date(TODAY_YMD) ? TODAY_YMD : eom;
  ...
  /* ---- merge approved corrections client-side (times + mode) ---- */
  const rowsWithCorrections = useMemo(() => {
    if (!rows.length || !corrList.length) return rows;

    // pick the latest approved correction per date
    const approvedByDate = new Map();
    corrList.forEach((c) => {
      const status = (c.status || "").toLowerCase();
      const dateKey = c.for_date || c.date;
    ...
    return rows.map((r) => {
      ...
      // mode from reason or admin_note
      const newMode =
        decodeModeFromText(c.reason) ||
        decodeModeFromText(c.admin_note) ||
        null;

      if (!newIn && !newOut && !newMode) return r;

      return {
        ...r,
        check_in: newIn || r.check_in,
        check_out: newOut || r.check_out,
        mode: newMode || r.mode,
        status: "Present",
        tag: r.tag || "corrected",
        _patchedByCorrection: true,
      };
    });
  }, [rows, corrList]);
  ...
  const hasCheckOut = useMemo(() => displayRows.some((r) => r.check_out), [displayRows]);
  const hasLateMins = useMemo(() => displayRows.some((r) => typeof r.minutes_late === "number"), [displayRows]);
  const hasHours = useMemo(() => displayRows.some((r) => minutesFromRow(r) != null), [displayRows]);

  const openCorrectionFor = (row) => {
    const preMode = row?.mode === "WFH" || row?.mode === "Onsite" ? row.mode : "";
    setCorrForm({
      for_date: row?.date || "",
      requested_check_in: toHHMM(row?.check_in),
      requested_check_out: toHHMM(row?.check_out),
  ...
  const submitCorrection = async (e) => {
    ...
    // append [MODE:WFH] or [MODE:Onsite] to reason if selected
    let finalReason = corrForm.reason.trim();
    if (corrForm.mode) {
      finalReason = finalReason.replace(MODE_TAG_RE, "").trim();
      finalReason += (finalReason ? " " : "") + `[MODE:${corrForm.mode}]`;
    }
    ...
    <div className="w-full min-h-screen bg-gray-100 px-6 py-10">
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-gray-100">
              <tr>
                ...
                <th className="px-4 py-2 text-left">Req. check-in</th>
                <th className="px-4 py-2 text-left">Req. check-out</th>
                <th className="px-4 py-2 text-left">Requested Mode</th>
                <th className="px-4 py-2 text-left">Status</th>
                <th className="px-4 py-2 text-left">Admin note</th>
              </tr>
            </thead>
            <tbody>
              {corrList.map((c) => {
                ...
                const reqMode = decodeModeFromReason(c.reason) || "—";
                ...
                return (
                  <tr key={c.id ?? `${d}-${rci}-${rco}`} className="border-t">
                    ...
                    <td className="px-4 py-2">{rci || "—"}</td>
                    <td className="px-4 py-2">{rco || "—"}</td>
                    <td className="px-4 py-2">{reqMode}</td>
                    <td className="px-4 py-2">
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${statusCls}`}>
                        {c.status || "pending"}
                      </span>
      ...
      <Modal open={corrOpen} onClose={() => setCorrOpen(false)} title="Request time correction">
        <form onSubmit={submitCorrection} className="grid gap-4">
          ...
          <div>
            <label className="block text-sm text-gray-600 mb-1">Mode (optional)</label>
            <select
              value={corrForm.mode || ""}
              onChange={(e) => setCorrForm((p) => ({ ...p, mode: e.target.value }))}
              className="border rounded-lg px-3 py-2 w-full"
            >
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="border rounded-lg px-3 py-2 w-full"
              placeholder="What needs to be fixed and why? (Optionally mention WFH/Onsite reason)"
            />
          </div>
