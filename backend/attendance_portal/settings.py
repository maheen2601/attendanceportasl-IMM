...
def env_list(k, default=""): return [x.strip() for x in (env(k, default) or "").split(",") if x.strip()]
def env_bool(k, default="False"): return (env(k, default) or "").strip().lower() in {"1","true","yes","on"}

BASE_DIR = Path(__file__).resolve().parent.parent
RENDER_HOST = env("RENDER_EXTERNAL_HOSTNAME", "")  # set by Render

# ---------- security / debug ----------
SECRET_KEY = env("SECRET_KEY", "dev-secret-change-me")
DEBUG = env_bool("DEBUG", "False")

# Be explicit in prod; .onrender.com wildcard is fine for dev
ALLOWED_HOSTS = env_list("ALLOWED_HOSTS", ".onrender.com,localhost,127.0.0.1")
if RENDER_HOST and RENDER_HOST not in ALLOWED_HOSTS:
    ALLOWED_HOSTS.append(RENDER_HOST)
...
MIDDLEWARE = [
    "corsheaders.middleware.CorsMiddleware",             # must be first of custom middlewares
    "django.middleware.security.SecurityMiddleware",
...
# ---------- CORS / CSRF ----------
# Explicit frontend URL (optional helper)
FRONTEND_URL = (env("FRONTEND_URL", "") or "").rstrip("/")
...
if FRONTEND_URL.startswith(("http://","https://")) and FRONTEND_URL not in CSRF_TRUSTED_ORIGINS:
    CSRF_TRUSTED_ORIGINS.append(FRONTEND_URL)
if RENDER_HOST:
    CSRF_TRUSTED_ORIGINS = list({*CSRF_TRUSTED_ORIGINS, f"https://{RENDER_HOST}"})
...
if DATABASE_URL:
    DATABASES = {
        "default": dj_database_url.parse(
            DATABASE_URL,
            conn_max_age=600,
            ssl_require=False  # Internal Render URL is inside private network
        )
    }
else:
    DATABASES = {
...
DEFAULT_FROM_EMAIL  = env("DEFAULT_FROM_EMAIL", "AdminHub <pc19463.malikfarooq@gmail.com>")

# comma-separated list -> python list
NOTIFY_ADMIN_EMAILS = [e.strip() for e in env("NOTIFY_ADMIN_EMAILS","").split(",") if e.strip()]
