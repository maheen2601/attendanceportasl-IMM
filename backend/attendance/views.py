...
# We’ll hide the requested mode inside the reason using a token like: [MODE:WFH]
_MODE_TOKEN_RE = re.compile(r"\[MODE:(WFH|Onsite)\]", re.I)

def _encode_mode_in_reason(reason: str, mode: str | None) -> str:
    """Append [MODE:WFH|Onsite] to reason (idempotent)."""
    reason = (reason or "").strip()
    if not mode:
        return reason
    # remove any existing token first to avoid duplicates
    reason_wo = _MODE_TOKEN_RE.sub("", reason).strip()
    token = f"[MODE:{mode}]"
    return (reason_wo + ("\n" if reason_wo else "") + token).strip()
...
def my_attendance_corrections(request):
    ...
    # NEW: optional mode
    want_mode = (request.data.get("want_mode") or "").strip()
    if want_mode and want_mode not in {"WFH", "Onsite"}:
        return Response({"detail": "want_mode must be 'WFH' or 'Onsite'."}, status=400)
    ...
    # embed requested mode (no DB change)
    reason = _encode_mode_in_reason(reason, want_mode if want_mode else None)
...
class AttendanceCorrectionAdminUpdate(RetrieveUpdateAPIView):
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        if prev != "approved" and instance.status == "approved":
            ...
            # ✅ NEW: apply requested mode from reason text (no DB change)
            req_mode = _decode_mode_from_text(instance.reason)
            if req_mode in ("WFH", "Onsite"):
                att.mode = req_mode

            shift_start = timezone.make_aware(datetime.combine(instance.for_date, emp.shift_start))
            att.minutes_late = max(
                0, int(((att.check_in or shift_start) - shift_start).total_seconds() // 60)
            )
            att.tag = "corrected"
            att.status = "Present"
            att.save()
